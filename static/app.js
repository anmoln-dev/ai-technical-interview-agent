...
async function submitAnswer() {
  ...
  try {
    ...
    else if (data.live_test_requested_by_ai) openLiveTestModalUnscripted();
    if (state.ttsEnabled) speakText(data.agent_response);
    if (data.is_complete) setTimeout(() => fetchFeedbackReport(), 2500);
  } catch (err) {
...
function renderFeedbackReport(report) {
  ...
  let aiBanner = document.getElementById('report-ai-generated-banner');
  if (!aiBanner) {
    aiBanner = document.createElement('p');
    aiBanner.id = 'report-ai-generated-banner';
    aiBanner.style.cssText = 'font-size:0.78rem; margin-top:0.5rem; text-align:center;';
    const scoreEl = document.getElementById('report-overall-score');
    if (scoreEl) scoreEl.parentElement.appendChild(aiBanner);
  }
  if (report.ai_generated) {
    aiBanner.innerHTML = '<span style="color:var(--accent-emerald);">⚡ This report was generated by Gemini AI based on your actual interview conversation.</span>';
    aiBanner.style.display = '';
  } else {
    aiBanner.innerHTML = '<span style="color:var(--text-muted);">💡 Demo report — based on simulated scoring heuristics.</span>';
    aiBanner.style.display = '';
  }
}
...
// Open the code editor in unscripted (AI-driven) mode — no challenge object needed.
// The AI already described the challenge in the chat; this just opens the editor.
function openLiveTestModalUnscripted() {
  const modal = document.getElementById('modal-live-test');
  if (!modal) return;

  document.getElementById('live-test-domain-badge').textContent = 'Live Coding Challenge';
  document.getElementById('modal-live-test-title').textContent = '💻 Live Code Challenge';
  document.getElementById('live-test-problem').textContent =
    'Your interviewer has described a coding challenge above. Write your solution in the editor below.';
  document.getElementById('live-test-code-editor').value =
  ...
  const timerEl = document.getElementById('live-test-timer');
  let timeRemaining = 420; // 7 minutes for open-ended
  function formatTime(secs) {
  ...
  liveTimerInterval = setInterval(() => {
    ...
    if (timeRemaining % 30 === 0 || timeRemaining <= 60)
      timerEl.setAttribute('aria-label', `Time remaining: ${timeRemaining} seconds`);
    if (timeRemaining <= 30 && timeRemaining > 0)
      announceSR(`Warning: ${timeRemaining} seconds remaining.`);
    if (timeRemaining <= 0) {
      clearInterval(liveTimerInterval);
      timerEl.textContent = '⏱️ 00:00';
      timerEl.setAttribute('aria-label', 'Time is up');
  ...
  openModal('modal-live-test', document.getElementById('btn-open-settings'));
  announceSR('Live code challenge triggered. Please write your solution in the editor.');
  document.getElementById('live-test-code-editor').focus();

  // Submit handler: just close the modal and post code as a chat message
  document.getElementById('btn-submit-live-code').onclick = async () => {
    const code = document.getElementById('live-test-code-editor').value;
    clearInterval(liveTimerInterval);
    closeModal('modal-live-test');
    // Post the code inline into the interview chat for the AI to evaluate
    const inputEl = document.getElementById('candidate-input-text');
    if (inputEl) {
      inputEl.value = `[Live Code Submission]\n\`\`\`python\n${code}\n\`\`\``;
      submitAnswer();
    }
  };
}
